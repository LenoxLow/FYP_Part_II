...
for index, tele in vtele.iterrows():
    ...
    elif (tele[' Longitude'] >= 103.3217 and tele[' Latitude'] >= 2.0308 and tele[' Longitude'] <= 103.3220 and
                tele[' Latitude'] <= 2.0312):
        print(index)
        vtele.set_value(index, 'Road Structure', 'Corner')
    elif (tele[' Longitude'] >= 103.3219 and tele[' Latitude'] >= 2.0304 and tele[' Longitude'] <= 103.32214 and
                tele[' Latitude'] <= 2.0306):
        print(index)
        vtele.set_value(index, 'Road Structure', 'Intersection')
    elif(tele[' Longitude'] >= 103.3235 and tele[' Latitude'] >= 2.0309 and tele[' Longitude'] <= 103.3238 and
                tele[' Latitude'] <= 2.0312):
        print(index)
        vtele.set_value(index, 'Road Structure', 'Intersection')
